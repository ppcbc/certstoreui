...
function DeleteFullExamCategory() {
    ...
    async function deleteExamsInFullCategory(fullCategoryId) {
        try {
            ...
            const categories = responseCategories.data.filter(item => item.fullCategoryId == fullCategoryId);
            ...
            for (const category of categories) {
                ...
                const exams = responseExams.data.filter(item => item.categoryId == category.categoryId);
            ...
            console.log(`Deleted all exams and categories in full category ${fullCategoryId}`);
        } catch (error) {
    ...
    async function onSubmit(e) {
        ...
        try {
            ...
            var response = await axios.delete(http + `api/FullCategories/${selectedFullCategory}`, {
                headers: {
                    Authorization: "Bearer " + myToken
                }
            });
            ...
            if (response.status === 204 || response.status === 200) {
                setMessage("Full category and associated categories and exams successfully deleted");
                handleMessage();
                setSelectedFullCategory("");
                fetchFullCategories();
            }
        } catch (error) {
            setMessage("Error deleting the full category and associated items, please try again");
            handleMessage();
    ...
    function validate() {
        let tempErrors = {};
        if (!selectedFullCategory) tempErrors.selectedFullCategory = "Please select a full category";
                ...
                <div className="delete-full-exam-category">
                    <div className="delete-box-FullExCate">
                        ...
                        {errors.selectedFullCategory && (
                            <p className="DeleteFullExamCategory-error-message">{errors.selectedFullCategory}</p>
                        )}
                        <div className="delete-my-inner-box-FullExCate">
                        ...
                        <div className="delete-my-label-FullExCate">{check && <p>{message}</p>}</div>
                    </div>
                </div>
            </form>
